...
/// <summary>
/// Implements the <see cref="IRepositoryFactory{TContext}"/> interface to provide instances of <see cref="IRepository{T}"/> objects that are specific to Entity Framework Core.
/// </summary>
    ...
    where TContext : DbContext
{
    ...
    public IRepository<T> Create<T>(IUnitOfWork<TContext> unitOfWork) where T : class
    {
        ...
        var repository = (IRepository<T>)ActivatorUtilities.CreateInstance(_serviceProvider,
            repositoryType,
            parameters: new object[] { unitOfWork });
